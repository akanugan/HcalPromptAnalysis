{
  ...
  TFile *_file0 = TFile::Open("pfstudy_histograms1.root");
  //  TFile *_file1 = TFile::Open("pfstudy_histograms_2019.root");
  _file0->ls();

  // - MC
  
  TH1F *pf_all    = (TH1F*) _file0->Get("All PFPar Response");
  TH1F *cHad_nHad    = (TH1F*) _file0->Get("cHad_nHad  Response");
  TH1F *dRmin    = (TH1F*) _file0->Get("dRmin Response");
  /*
  ...
  */
  ...
  tframe->GetYaxis()->SetTitle("Events");
  tframe->GetXaxis()->SetTitle("(PFParPt-GenParPt)/GenParPt");
  ...
  */
  ...
  //tframe->DrawCopy();
  pf_all->SetLineColor(kBlack);
  pf_all->Draw("same");
  cHad_nHad->SetLineColor(kRed);
  cHad_nHad->Draw("same");
  dRmin->SetLineColor(kBlue);
  dRmin->Draw("same");
  dRmin->GetXaxis()->SetTitle("(PFParPt-GenParPt)/GenParPt"); 

  TLegend *tl5 = new TLegend(0.1,0.7,0.3,0.9); // x1,y1,x2,y2
  //  tl5->SetHeader("Hadron Response","C");
  tl5->AddEntry(pf_all,"All PFPar Response","l");
  tl5->AddEntry(cHad_nHad,"Charged&Neutral hadron Response","l");
  tl5->AddEntry(dRmin,"min dR","l");
  tl5->Draw();
  ...
  */
  ...
  //c5->Print("c");
  c5->Print("Hadron_Response_GenptVsPFpt.pdf");
