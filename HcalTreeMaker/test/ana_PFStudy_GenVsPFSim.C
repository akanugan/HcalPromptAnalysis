   ...
   */
   ...
   while (fReader.Next()) {
     ...
     for (int igen = 0, ngen = GenParPt.GetSize(); igen<ngen; ++igen){
       if (fabs(GenParEta[igen])>2.4) continue;
       if (GenParPt[igen]<20) continue;
	 ...
	 if (deltaR_simhits <= 0.1){
	   SimEnergy += HcalSimHitsEnergy[isimhits];
	   SimHits = true;
	 }
       }
       if (SimHits) {
	 fill1D(v_hist, "SimHits Energy Response", ((HcalSimenergy[]-v_gen.Energy())/v_gen.Energy());
		}
       */

       for (int ipfsim = 0, npfsim =  PFSimParPt.GetSize(); ipfsim < npfsim; ++ipfsim) {
	 v_temp.SetPtEtaPhiM(PFSimParPt[ipfsim],PFSimParEta[ipfsim],PFSimParPhi[ipfsim],0);	 

	 if (v_temp.DeltaR(v_gen) < dR){ 
	   if (v_temp.DeltaR(v_gen)<dRmin){
	     dRmin = v_temp.DeltaR(v_gen);
	     v_pf_dRmin = v_temp;
	     PFParPdgId_dRmin = PFParPdgId[ipfsim];
	   }
	   v_pf_all += v_temp;
	   if ((PFParPdgId[ipfsim] == 5) || (PFParPdgId[ipfsim] == 1)) v_pf_ch_nh += v_temp;
	 }
       ...
       if(cHad){
	 fill1D(v_hist, "All PFPar Response", (v_pf_all.Pt()-v_gen.Pt())/v_gen.Pt());
	 fill1D(v_hist, "cHad_nHad  Response", (v_pf_ch_nh.Pt()-v_gen.Pt())/v_gen.Pt());
	 fill1D(v_hist, "dRmin Response", (v_pf_dRmin.Pt()-v_gen.Pt())/v_gen.Pt());
	 ...
	 //   for (int ipfsim = 0, npfsim =  PFParPt.GetSize(); ipfsim < npfsim; ++ipfsim) {
	 //     v_temp.SetPtEtaPhiM(PFParPt[ipfsim],PFParEta[ipfsim],PFParPhi[ipfsim],PFParM[ipfsim]);
	 //     if (v_temp.DeltaR(v_gen) < 0.4){ 
	 //       printf(" pf pt,eta,phi,mass,ID: %8.3f, %8.3f, %8.3f, %8.3f %5d\n",v_temp.Pt(),v_temp.Eta(),v_temp.Phi(),v_temp.M(),PFParPdgId[ipfsim]);
	 //     }
	 //   } // loop-over PF candidate ends
	 // } // if response<0.5
        // if debug
      //   if cHad
       ...
       float zero1=0.;
       if (PFParHcalFrac1[ipfsim]==0.) zero1=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero1);

       strtmp = "PFTask_hcalFrac2Zero_vs_pt";
       float zero2=0.;
       if (PFParHcalFrac2[ipfsim]==0.) zero2=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero2);

       strtmp = "PFTask_hcalFrac3Zero_vs_pt";
       float zero3=0.;
       if (PFParHcalFrac3[ipfsim]==0.) zero3=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero3);

       strtmp = "PFTask_hcalFrac4Zero_vs_pt";
       float zero4=0.;
       if (PFParHcalFrac4[ipfsim]==0.) zero4=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero4);

       strtmp = "PFTask_hcalFrac5Zero_vs_pt";
       float zero5=0.;
       if (PFParHcalFrac5[ipfsim]==0.) zero5=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero5);

       strtmp = "PFTask_hcalFrac6Zero_vs_pt";
       float zero6=0.;
       if (PFParHcalFrac6[ipfsim]==0.) zero6=1.;
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero6);

       strtmp = "PFTask_hcalFrac7Zero_vs_pt";
       float zero7=0.;
       if (PFParHcalFrac7[ipfsim]==0.) zero7=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]),zero7);

       strtmp = "PFTask_hcalFracAllZero_vs_pt";
       float zeroAll=0.;
       if (PFParHcalFrac1[ipfsim]==0.
	   && PFParHcalFrac2[ipfsim]==0. 
	   && PFParHcalFrac3[ipfsim]==0. 
	   && PFParHcalFrac4[ipfsim]==0. 
	   && PFParHcalFrac5[ipfsim]==0. 
	   && PFParHcalFrac6[ipfsim]==0. 
	   && PFParHcalFrac7[ipfsim]==0.) zeroAll=1.; 
       fill1DProf(v_hist, strtmp, log10(PFParPt[ipfsim]), zeroAll);

       //
       // Endcap
       // 
       if ( fabs(PFParEta[ipfsim])<2.9&&fabs(PFParEta[ipfsim])>1.5){

	 //
	 // http://cmslxr.fnal.gov/source/DataFormats/ParticleFlowCandidate/interface/Pfsimidate.h
	 //
	 // Charged hadrons
	 //
	 if ( PFParPdgId[ipfsim]==1 && fabs(PFParEta[ipfsim])<2.5 ){ 
	   if ( PFParPt[ipfsim]>5.){

	     if (debug) {
	     std::cout << PFParPt[ipfsim] << " " << PFParEta[ipfsim] << " " << PFParPdgId[ipfsim] << std::endl;
	     std::cout << PFParTrackPt[ipfsim] << " " << PFParEcalEnergyFrac[ipfsim] << " "
		       << PFParHcalEnergyFrac[ipfsim] << " " << PFParHOEnergyFrac[ipfsim] << std::endl;
	     std::cout << PFParHcalFrac1[ipfsim] << " "
		       << PFParHcalFrac2[ipfsim] << " " 
		       << PFParHcalFrac3[ipfsim] << " " 
		       << PFParHcalFrac4[ipfsim] << " " 
		       << PFParHcalFrac5[ipfsim] << " " 
		       << PFParHcalFrac6[ipfsim] << " " 
		       << PFParHcalFrac7[ipfsim] << std::endl;
	     std::cout << std::endl;
	     }
	   }
	   ...
	   if ( PFParPt[ipfsim]>5. ){
	     ...
	     fill1DProf(v_hist, strtmp, -3, PFParTrackPt[ipfsim]/PFParPt[ipfsim]);
	     fill1DProf(v_hist, strtmp, -2, PFParEcalEnergyFrac[ipfsim]);
	     fill1DProf(v_hist, strtmp, -1, PFParHcalEnergyFrac[ipfsim]);
	     fill1DProf(v_hist, strtmp, 1., PFParHcalFrac1[ipfsim]);
	     fill1DProf(v_hist, strtmp, 2., PFParHcalFrac2[ipfsim]);
	     fill1DProf(v_hist, strtmp, 3., PFParHcalFrac3[ipfsim]);
	     fill1DProf(v_hist, strtmp, 4., PFParHcalFrac4[ipfsim]);
	     fill1DProf(v_hist, strtmp, 5., PFParHcalFrac5[ipfsim]);
	     fill1DProf(v_hist, strtmp, 6., PFParHcalFrac6[ipfsim]);
	     fill1DProf(v_hist, strtmp, 7., PFParHcalFrac7[ipfsim]);
	     ...
	     fill1D(v_hist, strtmp, PFParTrackPt[ipfsim]/PFParPt[ipfsim]);

	     if (PFParTrackPt[ipfsim]!=PFParPt[ipfsim]){
...
// Main function
//
void ana_PFStudy_GenVsPFSim(TString rootfile="relval_SinglePi_2018_E50HCAL_GSR.root",TString outfile="pfstudy_histograms_GenVsPFSim.root",int maxevents=-1)
{
...
void bookHistograms(TList *v_hist)
{
  ...
  // book1D(v_hist, "SImHits Energy Response", 150, -1.5, 1.5);
