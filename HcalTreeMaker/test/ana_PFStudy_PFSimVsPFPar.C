   ...
   */
   ...
   while (fReader.Next()) {
     ...
     for (int ipfsim = 0, npfsim = PFSimParPt.GetSize(); ipfsim<npfsim; ++ipfsim){
       ...
       if (PFSimParPt[ipfsim] > PFSimParPtMax1){
	 //	 PFSimParPtMax2 = PFSimParPtMax1;
...
// Main function
//
void ana_PFStudy_PFSimVsPFPar(TString rootfile="relval_SinglePi_2018_E50HCAL_GSR.root",TString outfile="pfsimpar_v1_histograms.root",int maxevents=-1)
{
...
void bookHistograms(TList *v_hist)
{
  ...
  book1D(v_hist, "All PF with 1 Sim Par", 150, -1.5, 1.5);
